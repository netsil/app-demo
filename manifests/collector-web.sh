#!/bin/bash
if [ -z "${NETSIL_SP_HOST}" ] || [ -z "${NETSIL_ORGANIZATION_ID}" ] ; then
    ...
    wget --no-check-certificate --header="userport: 443" \
        ...
        && NETSIL_SP_HOST=${NETSIL_SP_HOST} NETSIL_ORGANIZATION_ID=${NETSIL_ORGANIZATION_ID} NETSIL_NICE_VALUE=15 SAMPLINGRATE=100 TAGS="apptype:mobile,apptier:web" /usr/bin/install-netsil-collectors.sh \
